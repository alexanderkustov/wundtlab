...
//Rare exceptions to indicate a bug in the _wundtlab code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_wundtlab = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_wundtlab]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.wundtlab.module"
